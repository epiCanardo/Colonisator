...
namespace Assets.Scripts.Front.MainManagers
{
    public class GameManager : UnityEngine.MonoBehaviour
    {
        ...
        void CreateSquaresTask()
        {
            ...
            for (int z = 0; z < 100; z++)
            {
                for (int x = 0; x < 100; x++)
                {
                    ...
                    {
                        if (ModManager.Instance.IsSquareNonNavigable(basicSquare.ToArray()))
                        {
        ...
        private List<SquareManagement> GetIslandCostalSquares(HarborSquareManagement harbor)
        {
            ...
            var nonNavagableSquaresCoordinates = ModManager.Instance.GetIslandCostalSquares(harbor.Island.harbourCoordinates.ToArray());
            foreach (var square in nonNavagableSquaresCoordinates)
            {
                SquareManagement squareM = GetPhysicalSquareFromSquare(new Square(square));
        ...
        private List<Square> GetFourNextNonNavigableSquares(Square square)
        {
            ...
            {
                if (ModManager.Instance.IsSquareNonNavigable(candidateSquare.ToArray()))
